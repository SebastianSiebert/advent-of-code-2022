...
module InputHandler =
    ...
    let (|IsMoves|) input = match input with | Moves m -> Some m | _ -> None
    let (|IsStackNumber|) input = match input with | StackNumber sn -> Some sn | _ -> None
    let (|IsStack|) input = match input with | Stack s -> Some s | _ -> None
    let chooseInput (|IsInput|_|) value = match value with | IsInput t -> Some t | _ -> None
...
let stackDefinition = inputs |> Seq.choose (chooseInput InputHandler.(|IsStackNumber|)) |> Seq.head
let stacks = inputs |> Seq.choose (chooseInput InputHandler.(|IsStack|)) |> Seq.rev |> Seq.fold fillStacks (createEmptyStacks stackDefinition)

let processMoves makeMovesFn stack = Seq.choose (chooseInput InputHandler.(|IsMoves|)) >> Seq.fold (makeMoves makeMovesFn) stack >> Seq.sortBy (fun s -> s.Index) >> Seq.map (fun e -> e.Stack.Head) >> System.String.Concat
